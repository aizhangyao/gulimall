 ...
 * @Description TODO
 * @Author ZhangYao
 * @Date Create in 17:46 2022/10/14
 * @Version 1.0
 */
@Slf4j
@Configuration
public class MyRabbitConfig {
     ...
     * 定制RabbitTemplate
     * 1、服务收到消息就会回调
     *      1、spring.rabbitmq.publisher-confirm-type: true
     *      2、设置确认回调
     * 2、消息正确抵达队列就会进行回调
     ...
     *     1、默认是自动确认的，只要消费端接受到，客户端会自动确认，服务端就会移除这个消息。
     *     问题：我们收到很多消息，自动回复给服务器ack，只有一个消息处理成功，宕机了。发生消息丢失。
     *     手动确认模式。只要我们没有明确告诉MQ，货物被签收，没有ACK，消息就一直是Unacked状态。
     *     即使Consumer宕机，消息不会丢失，会重新变成Ready，下一次有新的Consumer连接进来，就发给他。
     *
     */
